  ...
  if (!args[0])
    return message.channel.send(
      new Discord.MessageEmbed()
        .setColor("0x36393E")
        .setDescription(
          "> **Otorol sistemini ne yapmak istediğini belirtmedin** <a:YanpSnennleGif:727895115112251392> \n> `d!otorol ayarla @üye #kanal` **Otorol sistemini ayarlar.** \n> `d!otorol sıfırla` **Otorol sistemini sıfırlar.**"
        )
    );
  ...
  if (args[0] === "ayarla") {
    ...
    if (!rol)
      return message.channel.send(
        new Discord.MessageEmbed()
          .setColor("0x36393E")
          .setDescription(
            "**Bir rol etiketlemelisin** `d!otorol ayarla @rol #kanal` **(Eğer rolü bulamıyorsan etiketleme izninin açık olduğundan veya komutun kullanıldığı kanalı görebildiğinden emin ol)**"
          )
      );
    if (!rolkanal)
      return message.channel.send(
    ...
    return message.channel.send(
      new Discord.MessageEmbed()
        .setColor("0x36393E")
        .setDescription(
          "> **Otorol Sistemi Aktif edildi** <a:m_onay:819241950657904670> \n> **Sunucuya giren kişilere verilecek rol** <@&" +
            rol +
