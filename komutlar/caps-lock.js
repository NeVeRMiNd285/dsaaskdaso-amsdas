...
exports.run = async (client, message, args) => {   
  ...
  if (message.author.id !== ayarlar.sahip) return message.channel.send(`${prefix}capslock-engelle **aç** veya **kapat** yaz.`)
  if (!message.member.hasPermission("MANAGE_GUILD")) return message.channel.send('Bu komutu kullanmak için `SUNUCUYU_YÖNET` yetkisine sahip olman gerek!')
  ...
  var replace = await db.set(`capslockengl_${message.guild.id}`, 'kapali')
    ...
    .setColor('RANDOM')
    .setDescription('Capslock engelleme sistemi kapatıldı, herkes büyük harf kullanabilecek.')
   ...
   if (args[0] == 'aç') {
    ...
    .setColor('RANDOM')
    .setDescription('Capslock engelleme sistemi aktif, kimse büyük harf kullanamayacak')
